...
function CardServices({ title, text, image, className }: CardServicesProps) {
  return (
    <Card
      className={`${className} items-center bg-stone-200 dark:bg-zinc-800 rounded-md p-2 shadow-md overflow-hidden py-6 h-fit md:h-96 ring-0 border-0 text-foreground`}
    >
      <CardHeader className="w-full p-0">
        <h3 className="landing-heading text-center">{title}</h3>
      </CardHeader>
      <CardContent className="flex flex-col gap-4 md:flex-row items-center justify-evenly w-full p-0">
        <Image
          src={image}
        ...
        <div className="flex flex-col gap-4 items-center xl:w-1/2">
          <p className="landing-body text-start max-w-96">{text}</p>
        </div>
      </CardContent>
    </Card>
  );
